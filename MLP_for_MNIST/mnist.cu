...
__global__ void cross_entropy_backward(int w, int h, float *preds, float *real, float *output){
	int col = blockIdx.x * blockDim.x + threadIdx.x;
	int row = blockIdx.y * blockDim.y + threadIdx.y;
	if (row<h && col<w){
...
__global__ void init_rand(int w, int h, float *out){
	...
	if(row<h && col<w){
		curandState state;
		curand_init(44,row*w+col,0,&state);
		out[row*w+col] = curand_normal(&state)*sqrtf(2.f/h);
	}
}

void print_matrix(int w, int h, float *matrix, std::string name){
...
void init_layer(float *weights, float *biases, int w, int h, int BLOCK_SIZE){
...
int main(int argc, char **argv){
	...
	{
		Timer init("Layer Initialization");
	...
	for(int epoch=0; epoch<EPOCHS; epoch++){
		...
		for(int batch=0; batch<train_length/BATCH_SIZE; batch++){
            ...
            dimGrid  = dim3(ceil(size1/(float)BLOCK_SIZE),BATCH_SIZE/(float)BLOCK_SIZE,1);
			dimBlock = dim3(BLOCK_SIZE,BLOCK_SIZE,1);

			forward_relu<<<dimGrid,dimBlock>>>(BATCH_SIZE, input_size, size1, input, weights1, biases1, a1);
			gpuErrchk(cudaPeekAtLastError());

			dimGrid  = dim3(ceil(size2/(float)BLOCK_SIZE),BATCH_SIZE/(float)BLOCK_SIZE,1);
			dimBlock = dim3(BLOCK_SIZE,BLOCK_SIZE,1);

			forward_relu<<<dimGrid,dimBlock>>>(BATCH_SIZE, size1, size2, a1, weights2, biases2, a2);
			gpuErrchk(cudaPeekAtLastError());

			dimGrid  = dim3(ceil(size3/(float)BLOCK_SIZE),BATCH_SIZE/(float)BLOCK_SIZE,1);
			dimBlock = dim3(BLOCK_SIZE,BLOCK_SIZE,1);
			...
			cross_entropy_backward<<<dimGrid, dimBlock>>>(size3, BATCH_SIZE, a3, labels, d_l3);
			gpuErrchk(cudaPeekAtLastError());
